...
bool PhysicsEngine::CircleCircleCheck(CollisionCircle* circle1, CollisionCircle* circle2)
{
	...
	if (distance <= sum_radius)
	{
		...
		//Step 2. 
		glm::vec2 relative_velocity = circle2->GetRigidBody()->velocity - circle1->GetRigidBody()->velocity;
		float closing = Util::Dot(relative_velocity, contact_normal);

		glm::vec2 p1 = circle1->GetRigidBody()->mass * circle1->GetRigidBody()->velocity;
		glm::vec2 p2 = circle2->GetRigidBody()->mass * circle2->GetRigidBody()->velocity;
