...
void PlayScene::Draw()
{
	...
	auto GravityVector = PhysicsEngine::Instance().gravityVector;
	...
	//Force of Normal
	glm::vec2 ForceNormal = -(Util::Dot(ForceGravity, PlaneNormalVector) * PlaneNormalVector);

	
	//Force of Friction
	glm::vec2 ForceFriction = -(ForceGravity + ForceNormal);
...
void PlayScene::GUI_Function() 
{
	...
	float gravity = PhysicsEngine::Instance().gravityVector.y;
	if (ImGui::SliderFloat("Gravity", &gravity, 0.0f, 50.0f))
	{
		PhysicsEngine::Instance().gravityVector.y = gravity;
	}

	ImGui::SliderFloat("Mass", &m_pBall->GetRigidBody()->mass, 0.0f, 100.0f);
